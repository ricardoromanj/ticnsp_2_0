class TutorDatatable < AjaxDatatablesRails::Base
  ...
  def searchable_columns
    # Declare strings in this format: ModelName.column_name
    @searchable_columns ||= %w(User.name User.lastname User.gender User.email User.role)
  end
  ...
  def data
    records.map do |record|
      image = "<img src=#{record.current_image(30,30)} class='img-circle pull-right' alt='#{record.name} image' style='margin-left: 15px;'/>"
      show = "<a style= 'margin: 0 .5em;' class='btn btn-xs bg-teal' href='/users/#{record.id}'>Show</a>"
      edit = "<a style= 'margin: 0 .5em;' class='btn btn-xs btn-default' href='/users/#{record.id}/edit_tutor'>Edit</a>"
